...
from sawyer_nbv_env.utils import euler_from_quaternion, euler_to_quaternion

class CaptureDataset(SingleArmEnv):
    """
    ...
    def _load_model(self):
        ...
        for i in range(self.n_cameras):
            ...
            d_cam_pt /= d_cam_pt_length # normalised vector from camera towards the table centre
            qxyz = np.cross(cam_dir, d_cam_pt) 
            qw = 1 + np.dot(cam_dir, d_cam_pt)
            q = np.array([qxyz[0],qxyz[1],qxyz[2],qw])
            q /= np.linalg.norm(q)

            outputq = [q[2],q[1],-q[0],q[3]]
        
            # Add randomness to the orientation
            r,p,y = euler_from_quaternion(*outputq)
            kr = 0.2
            #r = np.random.uniform(-np.pi/8,np.pi/8)
            p+=np.random.uniform(-kr*d_cam_pt_length,kr*d_cam_pt_length)
            y+=np.random.uniform(-kr*d_cam_pt_length,kr*d_cam_pt_length)        
